...
const campaign = "cc-director-v3";
const outRoot = path.join(root, "AppStoreAssets");
const campaignDir = path.join(outRoot, "generated-marketing", campaign);
const zhDir = path.join(outRoot, "zh-Hans-cc-director-v3");
const enDir = path.join(outRoot, "en-US-cc-director-v3");
...
function sceneBody(data, locale, index) {
  ...
  if (data.scene === "hero") {
    return `
      ...
      ${focusCompass(data)}
      ${deviceHtml(source, "hero-phone tilt-back", 0.54)}
  ...
  if (data.scene === "single") {
    return `
      ${deviceHtml(source, "single-phone tilt-front", 0.63)}
  ...
  if (data.scene === "split") {
    ...
    return `
      ${deviceHtml(source, "split-phone tilt-back", 0.58)}
  ...
  if (data.scene === "map") {
    return `
      ${deviceHtml(source, "map-phone tilt-front", 0.56)}
  ...
  if (data.scene === "rooms") {
    ...
    return `
      ${deviceHtml(source, "rooms-phone tilt-back", 0.58)}
      <div class="room-stack">${rooms.map((room, i) => `<div style="--i:${i}">${escapeHtml(room)}<span></span></div>`).join("")}</div>
      <div class="soft-lens"></div>`;
  }
  return `
    ${deviceHtml(source, "final-phone-one tilt-back", 0.58)}
    ${deviceHtml(source2, "final-phone-two tilt-front", 0.43)}
...
.device-shadow{position:absolute;inset:3.2% 2.8%;border-radius:12%/6%;background:rgba(0,0,0,.46);filter:blur(62px);transform:translateY(62px) scale(.96)}
...
.device-reflection{position:absolute;left:0;right:0;top:91%;height:36%;transform:scaleY(-1);opacity:.22;filter:blur(.9px)}
...
.compass-disc{position:absolute;z-index:16;left:126px;top:1010px;width:610px;height:610px;border-radius:50%;padding:30px;background:radial-gradient(circle at 34% 26%,#fff7d9,#d6b66c 22%,#4d473a 66%,#191b19 100%);box-shadow:0 56px 140px rgba(0,0,0,.35),inset 0 0 0 2px rgba(255,244,210,.42)}
...
.hero-phone{right:112px;top:900px}.hero-readout{position:absolute;z-index:22;left:155px;top:1615px;width:500px;padding:28px 34px;border-radius:26px;background:rgba(18,24,21,.78);border:1px solid rgba(220,185,106,.44);box-shadow:0 28px 70px rgba(0,0,0,.26);backdrop-filter:blur(14px);color:#fbf4e2}
.hero-readout b{font-size:46px}.hero-readout span{display:block;margin-top:8px;font-size:24px;color:#d9ceb6;font-weight:650}
.single-phone{right:128px;top:905px}.reading-card{position:absolute;z-index:23;left:92px;width:360px;padding:30px 34px;border-radius:30px;background:rgba(255,250,236,.75);border:1px solid rgba(222,190,118,.48);box-shadow:0 28px 88px rgba(31,25,14,.19);backdrop-filter:blur(18px)}
...
.split-phone{right:132px;top:970px}.grid-block{position:absolute;z-index:22;left:94px;top:910px;display:grid;grid-template-columns:repeat(3,152px);gap:13px}
...
.map-phone{right:152px;top:1090px}.floor-plan{position:absolute;z-index:20;left:96px;top:920px;width:520px;height:520px;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;border:2px solid rgba(61,55,39,.52);box-shadow:0 28px 80px rgba(38,31,16,.16)}
...
.rooms-phone{right:142px;top:970px}.room-stack{position:absolute;z-index:22;left:92px;top:915px;display:flex;flex-direction:column;gap:25px}
...
.final-phone-one{right:136px;top:910px}.final-phone-two{left:110px;top:1220px}.closing-mark{position:absolute;z-index:25;left:90px;bottom:315px;width:670px;color:#fbf4e2}
