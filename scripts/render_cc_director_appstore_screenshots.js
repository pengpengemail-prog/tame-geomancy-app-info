...
const campaign = "cc-director-v1";
const outRoot = path.join(root, "AppStoreAssets");
const campaignDir = path.join(outRoot, "generated-marketing", campaign);
const zhDir = path.join(outRoot, "zh-Hans-cc-director-v1");
const enDir = path.join(outRoot, "en-US-cc-director-v1");
...
function sceneBody(data, locale, index) {
  ...
  if (data.scene === "hero") {
    return `
      ...
      ${focusCompass(data)}
      ${deviceHtml(source, "hero-phone tilt-back", 0.86)}
  ...
  if (data.scene === "single") {
    return `
      ${deviceHtml(source, "single-phone tilt-front", 0.96)}
  ...
  if (data.scene === "split") {
    ...
    return `
      ${deviceHtml(source, "split-phone tilt-back", 0.84)}
  ...
  if (data.scene === "map") {
    return `
      ${deviceHtml(source, "map-phone tilt-front", 0.88)}
  ...
  if (data.scene === "rooms") {
    ...
    return `
      ${deviceHtml(source, "rooms-phone tilt-back", 0.90)}
      <div class="room-stack">${rooms.map((room, i) => `<div style="--i:${i}">${escapeHtml(room)}<span></span></div>`).join("")}</div>
      <div class="soft-lens"></div>`;
  }
  return `
    ${deviceHtml(source, "final-phone-one tilt-back", 0.78)}
    ${deviceHtml(source2, "final-phone-two tilt-front", 0.62)}
...
h1{margin:0;font-size:92px;line-height:.98;letter-spacing:0;font-weight:850;color:#151614}
.en h1{font-size:78px;line-height:1.02}
...
.device-reflection{position:absolute;left:0;right:0;top:94%;height:34%;transform:scaleY(-1);opacity:.26;filter:blur(.7px)}
...
.stage-plinth{position:absolute;z-index:8;left:160px;top:1845px;width:940px;height:260px;border-radius:50%;background:radial-gradient(ellipse at center,rgba(241,219,164,.34),rgba(0,0,0,.08) 46%,transparent 71%);filter:blur(2px)}
.compass-disc{position:absolute;z-index:16;left:75px;top:965px;width:790px;height:790px;border-radius:50%;padding:36px;background:radial-gradient(circle at 34% 26%,#fff7d9,#d6b66c 22%,#4d473a 66%,#191b19 100%);box-shadow:0 56px 140px rgba(0,0,0,.35),inset 0 0 0 2px rgba(255,244,210,.42)}
...
.hero-phone{right:24px;top:690px}.hero-readout{position:absolute;z-index:22;left:154px;top:1765px;width:500px;padding:28px 34px;border-radius:26px;background:rgba(18,24,21,.78);border:1px solid rgba(220,185,106,.44);box-shadow:0 28px 70px rgba(0,0,0,.26);backdrop-filter:blur(14px);color:#fbf4e2}
.hero-readout b{font-size:46px}.hero-readout span{display:block;margin-top:8px;font-size:24px;color:#d9ceb6;font-weight:650}
.single-phone{right:72px;top:690px}.reading-card{position:absolute;z-index:23;left:90px;width:348px;padding:30px 34px;border-radius:30px;background:rgba(255,250,236,.75);border:1px solid rgba(222,190,118,.48);box-shadow:0 28px 88px rgba(31,25,14,.19);backdrop-filter:blur(18px)}
...
.split-phone{right:50px;top:780px}.grid-block{position:absolute;z-index:22;left:94px;top:940px;display:grid;grid-template-columns:repeat(3,152px);gap:13px}
...
.callout-panel{position:absolute;z-index:22;left:94px;top:1535px;width:480px;padding:32px 36px;border-radius:30px;background:rgba(255,255,255,.68);border:1px solid rgba(190,164,96,.34);box-shadow:0 28px 80px rgba(43,38,25,.14);backdrop-filter:blur(18px)}
...
.map-phone{right:54px;top:1030px}.floor-plan{position:absolute;z-index:20;left:96px;top:920px;width:520px;height:520px;display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;border:2px solid rgba(61,55,39,.52);box-shadow:0 28px 80px rgba(38,31,16,.16)}
...
.rooms-phone{right:42px;top:720px}.room-stack{position:absolute;z-index:22;left:92px;top:930px;display:flex;flex-direction:column;gap:25px}
...
.final-phone-one{right:82px;top:735px}.final-phone-two{left:86px;top:1058px}.closing-mark{position:absolute;z-index:25;left:90px;bottom:315px;width:670px;color:#fbf4e2}
